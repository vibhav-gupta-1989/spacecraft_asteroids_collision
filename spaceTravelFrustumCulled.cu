...
#define ROWS 300  // Number of rows of asteroids.
#define COLUMNS 300 // Number of columns of asteroids.
...
AsteroidGPU* d_asteroids;
AsteroidGPU* h_asteroids;
int totalAsteroids = ROWS * COLUMNS;
...
void setup(void)
{
	...
	h_asteroids = new AsteroidGPU[totalAsteroids];
	...
	for (int j = 0; j < COLUMNS; j++)
	{
    	...
    	{
        	h_asteroids[idx].x = arrayAsteroids[i][j].getCenterX();
        	h_asteroids[idx].y = arrayAsteroids[i][j].getCenterY();
        	h_asteroids[idx].z = arrayAsteroids[i][j].getCenterZ();
        	h_asteroids[idx].r = arrayAsteroids[i][j].getRadius();
        	idx++;
    	}
	}

	cudaMalloc(&d_asteroids, totalAsteroids * sizeof(AsteroidGPU));
	cudaMemcpy(d_asteroids, h_asteroids,
           totalAsteroids * sizeof(AsteroidGPU),
...
__global__
void collisionKernel(AsteroidGPU* asteroids, int n,
                     float craftX, float craftZ, float angle,
                     int* collisionFlag)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;

    AsteroidGPU a = asteroids[idx];

    if (a.r <= 0.0f) return;

    float cx = craftX - 5 * sinf(angle * 0.0174533f);
    float cz = craftZ - 5 * cosf(angle * 0.0174533f);
    ...
    float rad = 7.072f + a.r;

    if (dist2 <= rad * rad)
    {
        atomicExch(collisionFlag, 1); // mark collision
    }
}

int asteroidCraftCollisionCUDA(float x, float z, float a)
{
    int h_flag = 0;
    int* d_flag;
    ...
    int threads = 1024;
    int blocks = (totalAsteroids + threads - 1) / threads;
...
void specialKeyInput(int key, int x, int y)
{
	...
	bool cpu_result = asteroidCraftCollision(tempxVal, tempzVal, tempAngle);
	...
	bool gpu_result = asteroidCraftCollisionCUDA(tempxVal, tempzVal, tempAngle);
	end = std::chrono::high_resolution_clock::now();
	std::chrono::duration<double> elapsed1 = end - start;
	
	std::cout << "GPU Time: " << elapsed1.count() << " seconds\n";

	std::cout << "Speedup: " << elapsed.count() / elapsed1.count() << std::endl;
